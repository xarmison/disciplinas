...
if __name__ == "__main__":
    ...
    model = NeuralNetwork(2, 5, 1, learning_rate=0.01,
                          epochs=10000, debug=False)
    ...
    if(model.debug):
        ...
        for i, item in enumerate(test_data):
            print("Prediction for {:15} is {:3}, expected {}".format(
                np.array_str(item), np.array_str(
                    np.round(model.predict(np.append(item, 1)))), test_labels[i]
            )
            )

    pred = np.round(model.predict(
        np.hstack((test_data, np.ones((test_data.shape[0], 1))))))
